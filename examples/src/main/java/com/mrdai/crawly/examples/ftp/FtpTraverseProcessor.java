...
import com.mrdai.crawly.network.ftp.FtpReply;
...
public class FtpTraverseProcessor implements PageProcessor {
    ...
    public ResultItems process(Response response) {
        ...
        for (FTPFile name : fetchedNames) {
            if (name.isDirectory()) {
                result.addRequest(new FtpCommand(fRequest.getHost(), FTPCmd.LIST, fRequest.getParameters() + "/" + name.getName()));
